...
def obtain_task_info(task_name):
    ...
    if task_name == "scissors":
        ...
        object_pose = [0, 0., 0.6, 0, 0, 0]
        goal_pose = [0, 0., 10.6, 0, 0, 0]
        another_object_pose = [0, 0, 10, 0, 0, 0]
        another_goal_pose = [0, 0, 10, 0, 0, 0]
        table_pose_dim = [0.0, 0.0, 0.5 * 0.6, 0.0, 0.0, 0.0, 0.5, 1.0, 0.6]
        object_asset_options = gymapi.AssetOptions()
        object_asset_options.density = 500
    ...
    if task_name == "switch":
        ...
        goal_pose = [0.0, 0.2, 10.65, 3.141592, 1.57, 0]
        another_object_pose = [0.0, 0.0, 10.0, 0.0, 0.0, 0.0]
        another_goal_pose = [0.0, 0.0, 10.0, 0.0, 0.0, 0.0]
        table_pose_dim = [0.0, 0.0, 0.5 * 0.55, 0.0, 0.0, 0.0, 0.5, 1.0, 0.55]
        object_asset_options = gymapi.AssetOptions()
        object_asset_options.density = 500
        object_asset_options.fix_base_link = True
        object_asset_options.use_mesh_materials = True
...
def compute_hand_reward(
    ...
    object_pos, object_rot, target_pos, target_rot, another_object_pos, another_object_rot, another_target_pos, another_target_rot, object_left_handle_pos, object_right_handle_pos, object_dof_pos,
...
def obtain_task_dof_info(this_task, env_ids, num_each_envs, device):
    ...
    for i in range(len(env_ids)):
        ...
        if env_ids[i] >= num_each_envs * 2:
            task_dof_inits[i] = to_torch([-0.59, 0], device=device)
